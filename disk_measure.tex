...
\begin{document}
	...
	\todo[inline]{To establish uniform stability on the level of sublevel set homology, we need to find an upper bound of $W_2(\mu_{X,\alpha}, \mu_{X_0,\alpha})$ which is $\mathcal{O}(n^{-1})$. Maybe find an upper bound on the distance from each to a common measure $\nu$, then apply the triangle inequality for $W_2$?}
